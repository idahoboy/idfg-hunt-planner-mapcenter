...
return applyOpacity(color);
}`))}const St=me();function ma(a){a.vertex.code.add("#define noPerspectiveWrite(x, w) (x * w)")}function qe(a){a.fragment.code.add("#define noPerspectiveRead(x) (x * gl_FragCoord.w)")}function va(a){return a.pattern.map(e=>Math.round(e*a.pixelRatio))}function ga(a){if(a==null)return 1;const e=va(a);return Math.floor(e.reduce((t,i)=>t+i))}function Sa(a){return a==null?Yt:a.length===4?a:ve(ba,a[0],a[1],a[2],1)}const ba=Qe();function ya(a,e){if(!e.stippleEnabled)return void a.fragment.code.add(c`float getStippleAlpha(float lineWidth) { return 1.0; }
